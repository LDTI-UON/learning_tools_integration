...
class Settings {
  ...
  public static function get_instructor_settings() {
      ...
      $where = array("course_key" => static::$lti_module->course_key, "institution_id" => static::$lti_module->institution_id);

      ee()->db->where($where);
      $query = ee()->db->get("lti_instructor_settings");
  ...
  public static function get_plugin_setting($plugin_name, $parameter) {
        $q = static::get_instructor_settings();

        $arr = unserialize($q->plugins_active);

        return $arr[$plugin_name][$parameter];
  }
