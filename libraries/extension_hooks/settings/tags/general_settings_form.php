        ...
        foreach(static::$lti_plugins as $plugin) {
          if(!empty($plugin)) {
                if(isset($_POST["enable_$plugin"])) {
                     $plugins_active[$plugin] = array("active" => 1);
                } else if($token_valid) {
                     $plugins_active[$plugin] = array("active" => 0); // activate plugine by default
                }

                $form .= "<p>".$this->plugin_setup_text[$plugin."_description"];
                $form .= form_checkbox(array('name' => "enable_$plugin", 'id' => "enable_$plugin", 'value' => '1', 'checked' => isset($plugins_active[$plugin]) && $plugins_active[$plugin]['active'] == 1));
