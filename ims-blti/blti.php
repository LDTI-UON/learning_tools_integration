...
class BLTI {
    ...
    function __construct($parm=false, $usesession=true, $doredirect=true) {
        ...
        if ( is_string($parm) ) {
        	...
        	//var_dump(array('oauth_consumer_key' => $oauth_consumer_key, 'url_segment' => $url_segment));
        	$url_segment = $parm['url_segment'];
			$result = ee()->db->get_where('blti_keys', array('oauth_consumer_key' => $oauth_consumer_key, 'url_segment' => $url_segment));
        ...
        // expressionengine specific setup
		$uri = $parm['ee_uri'];

		// force SSL
		OAuthRequest::$FORCE_SSL = !empty($parm['force_ssl']) ? TRUE : FALSE;
    ...
    function getUserEmail() {
    	if(!isset($this->info['lis_person_contact_email_primary'])) return false;
    ...
    function getUserKey() {
        $_conk = $this->info['oauth_consumer_key'];
        $id = $this->info['user_id'];
        if ( strlen($id) > 0 && strlen($_conk) > 0 ) return $_conk . ':' . $id;
        return false;
    }
	...
	function getResourceLinkDescription() {
		return isset($this->info['resource_link_description'])?$this->info['resource_link_description']:"";
	}
	...
	function getToolConsumerContactEmail() {
		return $this->info['tool_consumer_instance_contact_email'];
	}
